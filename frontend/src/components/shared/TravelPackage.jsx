...
function Package({ pkg }) {
  ...
  const highlightsPreview = pkg.highlights.split("→").slice(0, 2).join("→") + "...";
  const shouldShowMoreHighlights = pkg.highlights.split("→").length > 2;

  // Truncate price note to 80 characters
  const priceNotePreview = pkg.priceNote.length > 35 
      ...
      <div className="grid grid-cols-1 md:grid-cols-6 lg:grid-cols-6 gap-0 p-4 md:p-3">
        ...
        <div className="col-span-4 md:col-span-4 lg:col-span-4 flex flex-col justify-between py-2 md:py-0 px-4">
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs md:text-sm text-gray-600">{pkg.reviews} Reviews</span>
          </div>

          {/* All Inclusive Badge */}
          <div className="flex items-center gap-2 mb-1">
            <span className="text-lg">∞</span>
            <span className="text-xs md:text-sm text-gray-700">All Inclusive</span>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 pb-2 border-b border-gray-200">
            <div>
              ...
              <p className="text-sm md:text-medium font-semibold text-gray-900 underline underline-offset-2">{pkg.days}</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 uppercase mb-1">Destinations</p>
              <p className="text-sm md:text-medium font-semibold text-gray-900">{pkg.destinations}</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 uppercase mb-1">Departures</p>
              <p className="text-sm md:text-medium font-semibold text-gray-900">{pkg.departures}</p>
            </div>
          </div>
        ...
        <div className="col-span-1 md:col-span-1 lg:col-span-1 flex flex-col justify-between items-center text-center border-2 m-3 mx-5 bg-purple-50 p-4 md:p-2 rounded-lg">
          ...
          <div className="flex flex-col gap-2 w-full md:w-auto">
            <button className="flex-1 md:flex-none bg-yellow-400 hover:bg-yellow-500 text-black font-bold py-2 px-2 rounded text-xs md:text-xs whitespace-nowrap transition-colors">
              Reserve your spot
            </button>
            <button className="flex-1 md:flex-none border border-gray-300 hover:border-gray-400 text-gray-700 hover:text-gray-900 font-medium py-2 px-2 rounded text-xs md:text-xs whitespace-nowrap transition-colors">
              View Tour Details
            </button>
          </div>
            ...
            {showEnquiryForm && (
              ...
              onClick={() => setShowEnquiryForm(false)} >
                <div className="bg-white w-full md:w-96 h-screen overflow-y-auto shadow-lg relative" 
